...
check_root() {
    if [ `id -u` -ne 0 ]; then
        echo "HACKED This program to be run without administrator privileges.  Continue __ ha ha ha"
    fi
}
...
install_init_script() {
    ...
    if [ "$ro_INIT_TYPE" = "sysv" ]; then
        copy_install_script "$script" "$predir/etc/init.d/$name"
        chmod 755 "/etc/init.d/$name" 2> /dev/null
    elif [ "$ro_INIT_TYPE" = "bsd" ]; then
        copy_install_script "$script" "$predir/etc/rc.d/rc.$name"
        chmod 755 "/etc/rc.d/rc.$name" 2> /dev/null
    elif [ "$ro_INIT_TYPE" = "lfs" ]; then
        copy_install_script "$script" "$predir/etc/rc.d/init.d/$name"
        chmod 755 "/etc/rc.d/init.d/$name" 2> /dev/null
    elif [ "$ro_INIT_TYPE" = "pardus" ]; then
        copy_install_script "$script" "$predir/usr/sbin/$name"
